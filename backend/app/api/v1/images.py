...
def success_response(
    data: object = None, message: str = "操作成功"
) -> dict[str, object]:
    """生成成功响应。"""
...
class ImagePullRequest(BaseModel):
    ...
    tag: str = Field(
        default="latest", min_length=1, max_length=100, description="镜像版本"
    )
...
async def search_repositories(
    ...
    try:
        ...
        return JSONResponse(
            content=success_response(data=repositories, message="搜索成功")
        )
...
async def list_tags(
    ...
    try:
        ...
        return JSONResponse(
            content=success_response(data={"tags": tags}, message="获取版本成功")
        )
    except ImageServiceError as exc:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def list_local_images(
    ...
    try:
        ...
        return JSONResponse(
            content=success_response(data=local_images, message="获取本地镜像成功")
        )
    except ImageServiceError as exc:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
...
@router.delete("/local", summary="删除本地镜像")
async def delete_local_image(
    image_ref: str,
    force: bool = False,
    service: ImageService = Depends(get_image_service),
    _current_user: User = Depends(get_current_user),
    ...
    """删除本地镜像。"""
    try:
        result = await service.remove_local_image(image_ref=image_ref, force=force)
        return JSONResponse(
            content=success_response(data=result, message="镜像删除成功")
        )
    ...
    except (ImageDeleteError, ImageServiceError) as exc:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content=error_response(exc.message, status.HTTP_400_BAD_REQUEST),
        )
...
async def ensure_image_available(
    ...
    try:
        ...
        return JSONResponse(
            content=success_response(data=availability, message="镜像可用")
        )
    except ImageNotFoundError as exc:
        return JSONResponse(
